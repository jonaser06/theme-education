...
if ($avg_author > 0 || $avg_users > 0) {
	...
	$output = $marks = $users = '';
	if ($use_tabs) {
		$author_tab = '<li class="sc_tabs_title"><a href="#author_marks" class="theme_button">'. esc_html__('Author', 'education').'</a></li>';
		$users_tab = '<li class="sc_tabs_title"><a href="#users_marks" class="theme_button">'. esc_html__('Users', 'education').'</a></li>';
	...
	if (!$reviews_first_author || !$reviews_second_hide) {
		...
		$field["descr"] = sprintf(__("Summary rating from <b>%s</b> user's marks.", 'education'), $users) . ' ' 
			.(!isset($_COOKIE['themerex_votes']) || themerex_strpos($_COOKIE['themerex_votes'], ','.($post_data['post_id']).',')===false
				? esc_html__('You can set own marks for this article - just click on stars above and press "Accept".', 'education')
				: esc_html__('Thanks for your vote!', 'education'));
		$field["accept"] = $allow_user_marks;
		$output .= '<div id="users_marks" class="sc_tabs_content'.(!$output ? ' screendisplayblock' : '') . '">' . trim(themerex_reviews_get_markup($field, $marks, $allow_user_marks, false, !$reviews_first_author)) . '</div>';
	}
	$reviews_markup .= $output . '</div>';
	if ($allow_user_marks) {
		wp_enqueue_script('jquery-ui-draggable', false, array('jquery', 'jquery-ui-core'), null, true);
		$THEMEREX_GLOBALS['js_vars']['reviews_allow_user_marks'] = ($allow_user_marks ? true : false);
		$THEMEREX_GLOBALS['js_vars']['reviews_max_level'] = $max_level;
		$THEMEREX_GLOBALS['js_vars']['reviews_levels'] = trim(themerex_get_theme_option('reviews_criterias_levels'));
		$THEMEREX_GLOBALS['js_vars']['reviews_vote'] = (isset($_COOKIE['themerex_votes']) ? $_COOKIE['themerex_votes'] : '');
		$THEMEREX_GLOBALS['js_vars']['reviews_marks'] = explode(",", $marks);
		$THEMEREX_GLOBALS['js_vars']['reviews_users'] = max(0, $users);
		$THEMEREX_GLOBALS['js_vars']['post_id'] = $post_data['post_id'];
	}
