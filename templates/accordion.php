...
if ( !function_exists( 'themerex_template_accordion_theme_setup' ) ) {
	...
	function themerex_template_accordion_theme_setup() {
		themerex_add_template(array(
			...
			'mode'   => 'blogger',
			'title'  => esc_html__('Blogger layout: Accordion (Style 1)', 'education')
			));
		themerex_add_template(array(
			'layout' => 'accordion-2',
			'template' => 'accordion',
			...
			'mode'   => 'blogger',
			'title'  => esc_html__('Blogger layout: Accordion (Style 2)', 'education')
			));
...
if ( !function_exists( 'themerex_template_accordion_output' ) ) {
	function themerex_template_accordion_output($post_options, $post_data) {
		?>
		<div class="post_item sc_blogger_item sc_accordion_item<?php if ($post_options['number'] == $post_options['posts_on_page'] && !themerex_sc_param_is_on($post_options['loadmore'])) { echo ' sc_blogger_item_last'; } ?>">
			...
			<div class="post_content sc_accordion_content">
				...
				if (themerex_sc_param_is_on($post_options['info'])) {
					?>
					<div class="post_info">
						<span class="post_info_item post_info_posted_by"><?php esc_html_e('Posted by', 'education'); ?> <a href="<?php echo esc_url($post_data['post_author_url']); ?>" class="post_info_author"><?php echo esc_html($post_data['post_author']); ?></a></span>
						<span class="post_info_item post_info_counters">
							<?php if ($post_options['orderby']=='comments' || $post_options['counters']=='comments') {
								esc_html_e('Comments', 'education');
							} else {
								esc_html_e('Views', 'education');
							} ?>
						}
							<span class="post_info_counters_number"><?php if ($post_options['orderby']=='comments' || $post_options['counters']=='comments') {
									themerex_show_layout($post_data['post_comments']);
								} else {
									themerex_show_layout($post_data['post_views']);
								} ?></span>
						</span>
					</div>
					<?php
