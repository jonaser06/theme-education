...
if (!$post_number) { 
	...
	if ($show_filters == 'categories') {			// Use categories as filter items
		...
		if (count($portfolio_list) > 0) {
			$filters .= '<a href="#" data-filter="*" class="'.esc_attr($filter_button_classes . ($portfolio_parent==$cur_term_id ? ' active' : '')) . '">' . esc_html__('All', 'education').'</a>';
		...
		if (count($flt_ids) > 0) {
			$filters .= '<a href="#" data-filter="*" class="'.esc_attr($filter_button_classes).' active">'. esc_html__('All', 'education').'</a>';
	...
	if ($filters) {
		$THEMEREX_GLOBALS['js_vars']['ppp'] = intval($ppp);
		$THEMEREX_GLOBALS['js_code'] = 'jQuery(".isotope_filters").append(' . $filters . ');';
	}
}
...
if ($post_number > 0) {
	...
	if (in_array($pagination, array('viewmore', 'infinite'))) {
		if ($page_number < $wp_query->max_num_pages) {
			...
			$THEMEREX_GLOBALS['js_vars']['viewmore_page'] = intval($page_number);
			$THEMEREX_GLOBALS['js_vars']['viewmore_data'] = str_replace("'", "\\'", serialize($args));
			$THEMEREX_GLOBALS['js_vars']['viewmore_vars'] = str_replace("'", "\\'", serialize(array(
			...
			<div id="viewmore" class="pagination_wrap pagination_<?php echo esc_attr($pagination); ?>">
				<a href="#" id="viewmore_link" class="theme_button viewmore_button"><span class="icon-spin3 animate-spin viewmore_loading"></span><span class="viewmore_text_1"><?php esc_html_e('LOAD MORE', 'education'); ?></span><span class="viewmore_text_2"><?php esc_html_e('Loading ...', 'education'); ?></span></a>
