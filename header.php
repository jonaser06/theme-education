...
>
	...
	<div class="body_wrap">
		...
		if ($video_bg_show) {
			...
			if (!empty($youtube)) {
				?>
				<div class="video_bg<?php echo esc_attr($overlay ? ' video_bg_overlay' : ''); ?>" data-youtube-code="<?php echo esc_attr($youtube); ?>"></div>
				<?php
			} else if (!empty($video)) {
				$info = pathinfo($video);
		...
		<div class="page_wrap">
			...
			if (themerex_get_custom_option('show_page_top') == 'yes') {
				...
				?>
				<div class="page_top_wrap<?php
					if ($show_title) {
						echo ' page_top_title';
					}
					if ($show_breadcrumbs) {
						echo ' page_top_breadcrumbs';
					} ?>"<?php themerex_show_layout($header_style); ?>>
