...
if ( !function_exists( 'themerex_register_theme_menus' ) ) {
	function themerex_register_theme_menus() {
		register_nav_menus(apply_filters('themerex_filter_add_theme_menus', array(
			'menu_main' => esc_html__('Main Menu', 'education'),
			'menu_user' => esc_html__('User Menu', 'education'),
			'menu_side' => esc_html__('Side Menu', 'education')
		)));
	}
}
	...
	function themerex_enqueue_slider($engine='all') {
		if ($engine=='all' || $engine=='swiper') {
			if (themerex_get_theme_option('debug_mode')=='yes' || themerex_get_theme_option('packed_scripts')=='no' || !file_exists(themerex_get_file_dir('css/__packed.css'))) {
				wp_enqueue_style( 'swiperslider-style', themerex_get_file_url('js/swiper/idangerous.swiper.css'), array(), null );
			}
			if (themerex_get_theme_option('debug_mode')=='yes' || themerex_get_theme_option('packed_scripts')=='no' || !file_exists(themerex_get_file_dir('js/__packed.js'))) {
				wp_enqueue_script( 'swiperslider-script', 			themerex_get_file_url('js/swiper/idangerous.swiper.js'), array('jquery'), null, true );
				wp_enqueue_script( 'swiperslider-scrollbar-script',	themerex_get_file_url('js/swiper/idangerous.swiper.scrollbar.js'), array('jquery'), null, true );
			}
		}
	}
}
...
if ( !function_exists( 'themerex_core_frontend_scripts_inline' ) ) {
	function themerex_core_frontend_scripts_inline() {
		...
		if (empty($THEMEREX_GLOBALS['js_vars']) || !is_array($THEMEREX_GLOBALS['js_vars'])) $THEMEREX_GLOBALS['js_vars'] = array();
		$vars = apply_filters('themerex_action_add_scripts_inline', $THEMEREX_GLOBALS['js_vars']) ;

		wp_localize_script('themerex-core-init-script', 'THEMEREX_GLOBALS', $vars);

		if (empty($THEMEREX_GLOBALS['js_code']) || !is_array($THEMEREX_GLOBALS['js_code'])) $THEMEREX_GLOBALS['js_code'] = '';
		$code = $THEMEREX_GLOBALS['js_code'];
...
if (!function_exists('themerex_core_add_scripts_inline')) {
	function themerex_core_add_scripts_inline($vars = array()) {
		...
		$vars['ajax_login']		= (themerex_get_theme_option('ajax_login')=='yes' ? true : false);

		// Site base url
		$vars['site_url']			= get_site_url();

		// Site protocol
		$vars['site_protocol']	= themerex_get_protocol();

		// VC frontend edit mode
		$vars['vc_edit_mode']		= (themerex_vc_is_frontend() ? true : false);
		...
		$vars['toc_menu']		= esc_attr(themerex_get_custom_option('menu_toc'));
		$vars['toc_menu_home']	= (themerex_get_custom_option('menu_toc')!='hide' && themerex_get_custom_option('menu_toc_home')=='yes' ? true : false);
		$vars['toc_menu_top']	= (themerex_get_custom_option('menu_toc')!='hide' && themerex_get_custom_option('menu_toc_top')=='yes' ? true : false);

		// Fix main menu
		$vars['menu_fixed']		= (themerex_get_theme_option('menu_position')=='fixed' ? true : false);
		...
		$vars['menu_slider']     = (themerex_get_theme_option('menu_slider')=='yes' ? true : false);
		...
		$vars['media_elements_enabled'] = (themerex_get_theme_option('use_mediaelement')=='yes' ? true : false);

		// Use AJAX search
		$vars['ajax_search_enabled'] 	= (themerex_get_theme_option('use_ajax_search')=='yes' ? true : false);
		...
		// Use CSS animation
		$vars['css_animation']      = (themerex_get_theme_option('css_animation')=='yes' ? true : false);
		$vars['menu_animation_in']  = esc_attr(themerex_get_theme_option('menu_animation_in'));
		$vars['menu_animation_out'] = esc_attr(themerex_get_theme_option('menu_animation_out'));

		// Popup windows engine
		$vars['popup_engine']	= esc_attr(themerex_get_theme_option('popup_engine'));
		$vars['popup_gallery']	= (themerex_get_theme_option('popup_gallery')=='yes' ? true : false);
		...
		$vars['remember_visitors_settings']	= (themerex_get_theme_option('remember_visitors_settings')=='yes' ? true : false);
...
if ( !function_exists( 'themerex_wp_title' ) ) {
	function themerex_wp_title( $title, $sep ) {
		...
		if ( $paged >= 2 || $page >= 2 )
			$title .= " $sep " . sprintf( esc_html__( 'Page %s', 'education' ), max( $paged, $page ) );
		return $title;
	}
}
