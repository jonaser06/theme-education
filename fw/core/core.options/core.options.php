...
if ( !function_exists( 'themerex_options_admin_menu_item' ) ) {
	function themerex_options_admin_menu_item() {
		...
		add_theme_page(__('Theme Options', 'education'), esc_html__('Theme Options', 'education'), 'edit_theme_options', 'themerex_options', 'themerex_options_page');
	}
}
...
if ( !function_exists( 'themerex_options_load_scripts' ) ) {
	function themerex_options_load_scripts() {
		...
		wp_enqueue_script( 'jquery-input-mask',				themerex_get_file_url('core/core.options/js/jquery.maskedinput.1.4.1.min.js'), array('jquery'), null, true );
...
if ( !function_exists( 'themerex_options_prepare_scripts' ) ) {
	function themerex_options_prepare_scripts($override='') {
		...
		$THEMEREX_GLOBALS['js_vars']['ajax_nonce'] 		= esc_attr(wp_create_nonce('ajax_nonce'));
		$THEMEREX_GLOBALS['js_vars']['ajax_url']		= esc_url(admin_url('admin-ajax.php'));
		$THEMEREX_GLOBALS['js_vars']['to_delimiter']	= esc_attr($THEMEREX_GLOBALS['to_delimiter']);
		$THEMEREX_GLOBALS['js_vars']['to_popup']		= esc_attr(themerex_get_theme_option('popup_engine'));
		$THEMEREX_GLOBALS['js_vars']['to_override']		= esc_attr($override);

		$keys = array();
		if (($export_opts = get_option('themerex_options_'.($override), false)) !== false) {
			$keys = join(',', array_keys($export_opts));
		}
		$THEMEREX_GLOBALS['js_vars']['to_export_list'] = $keys;

		if (empty( $THEMEREX_GLOBALS['js_vars']['to_strings'])) $THEMEREX_GLOBALS['js_vars']['to_strings'] = array();
		$THEMEREX_GLOBALS['js_vars']['to_strings']['del_item_error'] = esc_html__("You can't delete last item! To disable it - just clear value in field.", 'education');
		$THEMEREX_GLOBALS['js_vars']['to_strings']['del_item'] = esc_html__("Delete item error!", 'education');
		$THEMEREX_GLOBALS['js_vars']['to_strings']['wait'] = esc_html__("Please wait!", 'education');
		$THEMEREX_GLOBALS['js_vars']['to_strings']['save_options'] = esc_html__("Options saved!", 'education');
		$THEMEREX_GLOBALS['js_vars']['to_strings']['reset_options'] = esc_html__("Options reset!", 'education');
		$THEMEREX_GLOBALS['js_vars']['to_strings']['reset_options_confirm'] = esc_html__("You really want reset all options to default values?", 'education');
		$THEMEREX_GLOBALS['js_vars']['to_strings']['export_options_header'] = esc_html__("Export options", 'education');
		$THEMEREX_GLOBALS['js_vars']['to_strings']['export_options_error'] = esc_html__("Name for options set is not selected! Export cancelled.", 'education');
		$THEMEREX_GLOBALS['js_vars']['to_strings']['export_options_label'] = esc_html__("Name for the options set:", 'education');
		$THEMEREX_GLOBALS['js_vars']['to_strings']['export_options_label2'] = esc_html__("or select one of exists set (for replace):", 'education');
		$THEMEREX_GLOBALS['js_vars']['to_strings']['export_options_select'] = esc_html__("Select set for replace ...", 'education');
		$THEMEREX_GLOBALS['js_vars']['to_strings']['export_empty'] = esc_html__("No exported sets for import!", 'education');
		$THEMEREX_GLOBALS['js_vars']['to_strings']['export_options'] = esc_html__("Options exported!", 'education');
		$THEMEREX_GLOBALS['js_vars']['to_strings']['export_link'] = esc_html__("If need, you can download the configuration file from the following link: %s", 'education');
		$THEMEREX_GLOBALS['js_vars']['to_strings']['export_download'] = esc_html__("Download theme options settings", 'education');
		$THEMEREX_GLOBALS['js_vars']['to_strings']['import_options_label'] = esc_html__("or put here previously exported data:", 'education');
		$THEMEREX_GLOBALS['js_vars']['to_strings']['import_options_label2'] = esc_html__("or select file with saved settings:", 'education');
		$THEMEREX_GLOBALS['js_vars']['to_strings']['import_options_header'] = esc_html__("Import options", 'education');
		$THEMEREX_GLOBALS['js_vars']['to_strings']['import_options_error'] = esc_html__("You need select the name for options set or paste import data! Import cancelled.", 'education');
		$THEMEREX_GLOBALS['js_vars']['to_strings']['import_options_failed'] = esc_html__("Error while import options! Import cancelled.", 'education');
		$THEMEREX_GLOBALS['js_vars']['to_strings']['import_options_broken'] = esc_html__("Attention! Some options are not imported:", 'education');
		$THEMEREX_GLOBALS['js_vars']['to_strings']['import_options'] = esc_html__("Options imported!", 'education');
		$THEMEREX_GLOBALS['js_vars']['to_strings']['import_dummy_confirm'] = esc_html__("Attention! During the import process, all existing data will be replaced with new.", 'education');
		...
		<?php }	?>
				<div class="themerex_options_header">
					<div id="themerex_options_logo" class="themerex_options_logo">
						...
						<h2><?php esc_html_e('Theme Options', 'education'); ?></h2>
					</div>
					<div class="themerex_options_button_import"><span class="iconadmin-download"></span><?php esc_html_e('Import', 'education'); ?></div>
					<div class="themerex_options_button_export"><span class="iconadmin-upload"></span><?php esc_html_e('Export', 'education'); ?></div>
		<?php if ($to_flags['show_page_layout']) { ?>
					<div class="themerex_options_button_reset"><span class="iconadmin-spin3"></span><?php esc_html_e('Reset', 'education'); ?></div>
					<div class="themerex_options_button_save"><span class="iconadmin-check"></span><?php esc_html_e('Save', 'education'); ?></div>
		<?php }	?>
				</div>
	...
	function themerex_options_show_field($id, $field, $value=null) {
		...
		if ($field['type']=='media') {
			if (!isset($field['before'])) {
				$field['before'] = array(
					'title' => esc_html__('Choose image', 'education'),
					'action' => 'media_upload',
					'type' => 'image',
					'multiple' => false,
					'linked_field' => '',
					'captions' => array('choose' => esc_html__( 'Choose image', 'education'),
										'update' => esc_html__( 'Select image', 'education')
										)
				);
			}
		...
		if (themerex_options_is_group($field['type'])) {					// Close nested containers
			...
			if ($field['type'] != 'hidden') {
				...
				echo '<label class="themerex_options_field_label'.(!empty($THEMEREX_GLOBALS['to_flags']['add_inherit']) && isset($field['std']) ? ' themerex_options_field_label_inherit' : '').'" for="'.esc_attr($id).'">' . ($field['title']) 
					. (!empty($THEMEREX_GLOBALS['to_flags']['add_inherit']) && isset($field['std']) ? '<span id="'.esc_attr($id).'_inherit" class="themerex_options_button_inherit'.($inherit ? '' : ' themerex_options_inherit_off').'" title="' . esc_html__('Unlock this field', 'education') . '"></span>' : '')
					. '</label>'
					. "\n";
		...
		foreach ($field['val'] as $clone_num => $clone_val) {
			...
			case 'toggle':
				...
				echo '<h3 id="'.esc_attr($id).'_title"'
					...
					. '</h3>'
					. '<div class="themerex_options_content themerex_options_toggle_content'.(!empty($field['closed']) ? ' screendisplaynone"' : '').'">';
			break;
		...
		if (!themerex_options_is_group($field['type']) && $field['type'] != 'hidden') {
			if ($cloneable) {
				echo '<div class="themerex_options_input_button themerex_options_clone_button themerex_options_clone_button_add">'. esc_html__('+ Add item', 'education') .'</div>';
			}
			if (!empty($THEMEREX_GLOBALS['to_flags']['add_inherit']) && isset($field['std']))
				echo  '<div class="themerex_options_content_inherit'.($inherit ? '' : ' screendisplaynone').'"><div>'.esc_html__('Inherit', 'education').'</div><input type="hidden" name="'.esc_attr($id).'_inherit" value="'.esc_attr($inherit).'" /></div>';
			echo '</div>';
			if (!empty($field['desc']))
