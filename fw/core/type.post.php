		...
		if (isset($THEMEREX_GLOBALS['post_override_options']) && $THEMEREX_GLOBALS['post_override_options']['page']=='post') {
			$THEMEREX_GLOBALS['post_override_options']['fields'] = array(
					...
					"info_reviews_1" => array(
						"title" => esc_html__('Reviews criterias for this post', 'education'),
						"override" => "post",
						"divider" => false,
						"desc" => esc_html__('In this section you can put your reviews marks', 'education'),
						"class" => "reviews_meta",
						"type" => "info"),
					"show_reviews" => array(
...
if (!function_exists('themerex_post_add_options_column')) {
	function themerex_post_add_options_column( $columns ){
		$columns['theme_options'] = esc_html__('Theme Options', 'education');
		return $columns;
	}
}
		...
		if ($props = get_post_meta($post_id, 'post_custom_options', true)) {
			...
			foreach($props as $prop_name=>$prop_value) {
				if (!themerex_is_inherit_option($prop_value) && (!isset($THEMEREX_GLOBALS['options'][$prop_name]['type']) || $THEMEREX_GLOBALS['options'][$prop_name]['type']!='hidden')) {
					...
					$options .= '<div class="themerex_options_prop_row"><span class="themerex_options_prop_name">' . esc_html($prop_title) . '</span>&nbsp;=&nbsp;<span class="themerex_options_prop_value">' . (is_array($prop_value) ? esc_html__('[Complex Data]', 'education') : '"' . esc_html(themerex_strshort($prop_value, 80)) . '"') . '</span></div>';
				}
			}
		}
...
if (!function_exists('themerex_post_admin_scripts')) {
	function themerex_post_admin_scripts() {
		global $THEMEREX_GLOBALS;
		if (isset($THEMEREX_GLOBALS['post_override_options']) && $THEMEREX_GLOBALS['post_override_options']['page']=='post' && themerex_get_custom_option('show_reviews')=='yes')
...
if (!function_exists('themerex_post_before_show_override_options')) {
	function themerex_post_before_show_override_options($post_type, $post_id) {
		...
		$THEMEREX_GLOBALS['js_vars']['reviews_levels']			= themerex_get_theme_option('reviews_criterias_levels');
...
if ( !function_exists( 'themerex_post_list_post_types' ) ) {
	function themerex_post_list_post_types($list) {
		$list['post'] = esc_html__('Post', 'education');
		$list['page'] = esc_html__('Page', 'education');
		return $list;
	}
}
...
if ( !function_exists( 'themerex_post_get_blog_title' ) ) {
	function themerex_post_get_blog_title($title, $page) {
		...
		if (is_front_page()) {
			...
			$title = esc_html__( 'All Posts', 'education' );
		else if ( $page == 'author' ) {
			$curauth = (get_query_var('author_name')) ? get_user_by('slug', get_query_var('author_name')) : get_userdata(get_query_var('author'));
			$title = sprintf(esc_html__('Author page: %s', 'education'), $curauth->display_name);
		} else if ( $page == 'error404' )
			$title = esc_html__('URL not found', 'education');
		else if ( $page == 'search' )
			$title = sprintf( esc_html__( 'Search Results for: %s', 'education' ), get_search_query() );
		else if ( $page == 'archives_day' )
			$title = sprintf( esc_html__( 'Daily Archives: %s', 'education' ), themerex_get_date_translations(get_the_date()) );
		else if ( $page == 'archives_month' )
			$title = sprintf( esc_html__( 'Monthly Archives: %s', 'education' ), themerex_get_date_translations(get_the_date( 'F Y' )) );
		else if ( $page == 'archives_year' )
			$title = sprintf( esc_html__( 'Yearly Archives: %s', 'education' ), get_the_date( 'Y' ) );
			...
			$title = sprintf( esc_html__( 'Attachment: %s', 'education' ), themerex_get_post_title());
		else if ( $page == 'single' )
