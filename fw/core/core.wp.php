...
if (!function_exists('themerex_show_breadcrumbs')) {
	function themerex_show_breadcrumbs($args=array()) {
		...
		$etc = '<span class="breadcrumbs_hidden_items">&#9679;&#9679;&#9679;</span>';
			...
			if ($max_page > 1) {
				...
				if ($opt['style'] == 'pages') {
					...
					if (empty($opt['pages_text'])) 
						$opt['pages_text'] = esc_html__('Page %CURRENT_PAGE% of %TOTAL_PAGES%', 'education');
...
if ( !function_exists( 'themerex_callback_view_more_posts' ) ) {
	function themerex_callback_view_more_posts() {
		...
		if ($page > 0 && is_array($args) && is_array($vars)) {
			...
			$response['error'] = esc_html__('Wrong query arguments', 'education');
		}
...
if ( !function_exists( 'themerex_callback_ajax_search' ) ) {
	function themerex_callback_ajax_search() {
		...
		if (!empty($s)) {
			...
			if (empty($output)) {
				$output .= '<article class="post_item">' . esc_html__('Sorry, but nothing matched your search criteria. Please try again with some different keywords.', 'education') . '</article>';
			} else {
				$output .= '<article class="post_item"><a href="#" class="post_more search_more">' . esc_html__('More results &hellip;', 'education') . '</a></article>';
			}
			$response['data'] = $output;
		} else {
			$response['error'] = esc_html__('The query string is empty!', 'education');
		}
...
if (!function_exists('themerex_get_calendar')) {
	function themerex_get_calendar($onlyFirstLetter = false, $get_month = 0, $get_year = 0, $opt=array()) {
		...
		$calendar_caption = esc_html_x('%1$s %2$s', 'calendar caption', 'education');
		...
		for ( $day = 1; $day <= $daysinmonth; ++$day ) {
			...
			if ( !empty($posts[$day]) )
				$calendar_output .= '<a class="day_wrap" href="' . esc_url(!empty($posts[$day]['link']) ? $posts[$day]['link'] : get_day_link($thisyear, $thismonth, $day)) . '" title="' . esc_attr( is_int($posts[$day]['titles']) ? $posts[$day]['titles'].' '. esc_html__('items', 'education') : $posts[$day]['titles'] ) . "\">$day</a>";
			else
...
if (!function_exists('themerex_output_single_comment')) {
	function themerex_output_single_comment( $comment, $args, $depth ) {
		...
		switch ( $comment->comment_type ) {
			case 'pingback' :
				?>
				<li class="trackback"><?php esc_html_e( 'Trackback:', 'education' ); ?> <?php comment_author_link(); ?><?php edit_comment_link( esc_html__( 'Edit', 'education' ), '<span class="edit-link">', '<span>' ); ?>
				<?php
				break;
			case 'trackback' :
				?>
				<li class="pingback"><?php esc_html_e( 'Pingback:', 'education' ); ?> <?php comment_author_link(); ?><?php edit_comment_link( esc_html__( 'Edit', 'education' ), '<span class="edit-link">', '<span>' ); ?>
				<?php
				break;
			default :
				...
				<li id="comment-<?php comment_ID(); ?>" <?php comment_class('comment_item'); ?>>
					...
					<div class="comment_content">
						<div class="comment_info">
							...
							<span class="comment_date"><span class="comment_date_label"><?php esc_html_e('Posted', 'education'); ?></span> <span class="comment_date_value"><?php echo get_comment_date(get_option('date_format')); ?></span></span>
							<span class="comment_time"><?php echo get_comment_date(get_option('time_format')); ?></span>
						</div>
						<div class="comment_text_wrap">
							<?php if ( $comment->comment_approved == 0 ) { ?>
							<div class="comment_not_approved"><?php esc_html_e( 'Your comment is awaiting moderation.', 'education' ); ?></div>
							<?php } ?>
...
if (!function_exists('themerex_get_terms_by_taxonomy')) {
	function themerex_get_terms_by_taxonomy($tax_types = 'post_format', $args=array()) {
		...
		$terms = (array)get_terms($tax_types, $args);
		if (count($terms) > 0) {
			foreach ($terms as $k=>$v) {
				if (!empty($v) && empty($v->invalid_taxonomy)) {
...
if (!function_exists('themerex_get_terms_hierarchical_list')) {
	function themerex_get_terms_hierarchical_list($terms, $opt=array()) {
		...
		if (count($terms) > 0) {
			foreach ($terms as $term) {
				if (empty($v) || !isset($v['invalid_taxonomy'])) {
					continue;
				}
